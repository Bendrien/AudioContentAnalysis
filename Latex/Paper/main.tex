...
While in theory this can be used to find any kind of percussive event within a musical piece, we focused mainly on base drum hits in our research.
...
For syncing visuals to music in VJing or light show applications (Sound-to-Light) a drum transcription is also a integral part.
...
NMF is in its essence a pattern matching algorithm.
It decomposes the magnitude spectrum of a song into two matrices.
The first one contains the magnitude spectra of the individual components and the second one contains the time-varying gain of these components, which tells us when each of the components for the first matrix is active.
The sum of both matrices makes up an approximation of the original spectrum. \cite{virtanen2007}
...
It can for example be used to recognize parts of a face, find semantic features in text or certain events in a piece of music.
