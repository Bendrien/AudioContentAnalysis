...
For both inputs the first step is to apply the short-time Fourier transform (STFT) which transforms both signal from the time domain to the frequency domain.
...
%% TODO: explain transient and shit
...
\caption{Example of the representation of an activation matrix as a curve. With a red line marking the threshold. \scriptsize{\textsf{\textcopyright} Own work CC BY-SA 3.0}}
...
This value was establish empirically to separate the valid detection from false positives.
...
% TODO: zero crossings, transfrom from frame back to time, mention error range do to frame
