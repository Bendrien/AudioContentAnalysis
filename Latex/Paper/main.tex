...
While it is very easy for us humans to find patterns and percussive events in music such a task is very difficult to a computer.
In this research we show that it is possible for a computer to find the exact time points of drum events in a piece of music by using non-negative matrix factorization.
Generating such a so called drum transcription is useful in lots of different applications.
...
While in theory the proposed algorithm can be used to find any kind of event within a musical piece, we focused mainly on basic drum sounds in our research.

The transcription of a songs drum track can be useful in many cases.
...
For example using the results of the drum transcription a Beats per Minute (BPM) detection can be build, which in turn can then be the basis of a tempo detection or song segmentation algorithm.
...
The idea to use NMF for pattern recognition was first proposed in a paper by Lee and Seung in 1999 \cite{lee1999}.
...
In the field of MIR NMF has also been used in a variety of different applications.
...
T. Virtanen \cite{virtanen2007} among many others has researched this topic. % ich würde den satz drinn lassen, weil Virtanen ein Guru auf dem NMF gebiet ist. Name dropping und so!
%\cite{virtanen2007}
...
As mentioned above we used a \textbf{non-negative matrix factorization} to extract the time points of drum events from a drum track.
...
In our case we used the magnitude spectrum of a song as the $\mathbf{V}$ matrix and decompose it into two matrices.
The first one contains the magnitude spectra of the individual components we are interested in ($\mathbf{W}$) and the second one contains the time-varying gain of these components ($\mathbf{H}$), which tells us when each of the components form the first matrix is active.
...
The constraint of only using non-negative numbers is met since we are using magnitude spectra which by definition only contain non-negative number.
...
The NMF then solved for the matrix $\mathbf{H}$ (see figure \ref{fig:MatrixOverview} (c)) which told us at what point in time the specified base drum spectrum appears in the song.
In that setup the NMF essentially worked as a pattern matching algorithm.
...
The first input is a time domain signal of a percussive instrument, like a base drum or hi-hat, and the second input is a time domain signal of a piece of music.
...
A Hann window was applied to reduce spectral leakage.
...
After both inputs are prepared they were handed over to the NMF, which does its processing in the way described above.
The result of the NMF was in our case the activation matrix $\mathbf{H}$.
...
The next step of the algorithm is called \textbf{onset detection} where we extracted the exact time points from the matrix $\mathbf{H}$.
...
In order to do that the curve was first normalized.
Afterwards a threshold of $\frac{2}{3}$ was applied, which can be seen as a red line in figure \ref{fig:ActivationMatrix}.
This value was establish empirically to separate the valid detections from false positives.
Then the first derivative, which represents the slope of the curve, was calculated, by subtracting each element from the previous one.
An example of the resulting curve can be found in figure \ref{fig:Ableitung}.
In order to find the extract time points of a peak the zero crossings of the resulting curve had to be detected.
Once the frame number of a zero crossing was found, the frame number had to be converted back to a time point.
This was achieved by applying the following formula, where $f_s$ is the sample rate.
...
Due to the formula \ref{eq:timePoint} we were only able to get the time point within a range of approx. 12 ms (called epsilon).
Once this calculation was done we received a list of time points where the given template was found in the song which is the drum transcription we were looking for.
...
As our test data set we used the ENST-drums database kindly provided by the Institut Télécom Paris Tech\footnote{ENST drum test database: \url{https://perso.telecom-paristech.fr/~grichard/ENST-drums/}}.
...
For our purposes we use the mixed version of the tracks called ``wet-mix'' which also contains some acoustic room elements, like reverberation, making it more suitable as a test data set for real world applications.
...
This left us with a detection rate for each track.
...
From these results we can see that our algorithm returns very satisfying results for 2 out of 3 drum kits from our test data set.
...
It can only work as well as the templates it is provided with.
...
Here the empirically chosen threshold value of $\frac{2}{3}$ already yields good results, but it is also responsible for some missed events and some false positives.
Using an adaptive threshold would further improve the results.

As mentioned above the precision of the detected peak time point is only precise within a range of approx. 12 ms since only the STFT frame in which the peak occurs can be tracked.
